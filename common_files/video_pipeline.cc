...
int PipeLine::Create()
{
    ...
    if (ret) {
        printf("vb_set_config failed ret:%d, VB may already be initialized by another process, continuing...\n", ret);
    }
    ...
    ret = kd_mpi_vb_init();
    if (ret) {
        printf("vb_init failed ret:%d, VB may already be initialized, continuing...\n", ret);
    }
